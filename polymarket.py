...
async def discover_current_btc5m(now=None):
    ...
    # Try current window, next, and previous because event publishing can lag slightly.
    ...
    for start_ts in candidates:
        ...
        found.append({
            ...
            "market_numeric_id": str(m.get("id") or ""),
            "slug": slug,
            "title": ev.get("title") or "BTC Up or Down 5m",
            "start_ts": start_ts,
            "end_ts": start_ts + 300,
            "up_token_id": str(up),
    ...
    # Prefer a market containing now; otherwise nearest upcoming.
    current = [x for x in found if x["start_ts"] <= now < x["end_ts"]]
    if current:
        return current[0]
    upcoming = sorted([x for x in found if x["start_ts"] > now], key=lambda x:x["start_ts"])
    if upcoming:
        return upcoming[0]
    return sorted(found, key=lambda x:abs(x["start_ts"]-now))[0] if found else None
